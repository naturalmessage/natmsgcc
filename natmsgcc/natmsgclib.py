...
def verify_id_format(id, expected_prefix, version=1):
    ...
    if expected_prefix is None:
        return(print_err(20600, 'There was no expected_prefix ' 
    ...
    if (expected_prefix in ('SID', 'SMD', 'MIX')):
        ...
        if (len(id) == 35):
            ...
            return(print_err(21000, 'Length of a shard ID must be '
                + '35. Observed length was: ' \
                + str(len(id))))
            #raise Exception('Illegal ID length')
...
def nm_set_pgm_opt(opt_key, pgm_list, prompt1, prompt2):
    """Set an program option and save it to the config file."""
    global MAIN_CONFIG
...
def nm_find_default_config_fname():
    ...
    if os.path.isfile(unix_config_fname):
        ...
        if os.path.isfile(other_config_fname):
            ...
            if (os.stat(other_config_fname).st_size == 0):
                debug_msg(4, 'Config file is zero bytes: ' \
                    + other_config_fname)
                need_new_config_file = True
            else:
    ...
    if config_fname is None:
        ...
        debug_msg(4, '=== returning from nm_find_default_config_fname(): ' \
            + config_fname)
    ...
    if platform.system().lower() != 'windows':
        ...
        if root_owner_found:
            if os.getlogin() == 'root':
                ...
                owner_id_alpha = intput('Enter the user ID that you '
...
def nm_start(batch=False):
    ...
    if 'current_identity' not in MAIN_CONFIG['SETTINGS'].keys():
        print('WARNING. Your options file does not contain an '
    ...
    if 'download_directory' not in MAIN_CONFIG['SETTINGS'].keys():
        ...
        print('WARNING. Your options file does not contain '
        ...
        if pw_hashed is not None:
            if not need_new_receipt:
                ...
                if nm_confirm:
                    ...
                    need_new_receipt = False
                    print('')
                    print('Press any key to enter the password '
                        + 'again to verify')
