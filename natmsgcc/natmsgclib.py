...
# what was this.. Wed Feb 25 18:00:02 EST 2015
# Error 25200.  The HTML request for the shard read failed. The URL was https://106.187.53.102:4430/shard_read?shard_id=SID7B405042C018F6712791C1B21834481E. (<class 'requests.exceptions.SSLError'>, SSLError(SSLError(SSLEOFError(8, 'EOF occurred in violation of protocol (_ssl.c:598)'),),))

# A collection of libraries for 

# to do: 
#  implement unrtf on linux and bsd, and 'textutil -cat txt fname' to convert rtf to txt on mac os X
...
#NATMSG_ARCH_HEADER='Natural Message Archive 02.00'
...
#import pbkdf2_nm # A Natural Message module
...
def print_err(err_nbr, err_msg=None):
	...
	return(err_nbr)
########################################################################
##def debug_msgOLD(verbosity, test_val, msg, indent=0):
##	"""
...
##		if verbosity > 3:
##			leader = '  ' * indent
##			print(leader + 'DEBUG: ' + msg , file=sys.stderr)
##		else:
...
def validate_id_chars(id_chunk):
	...
	# and confirm that the characters are 0-9 or 
	...
	if (len(tmp2) != 0):
		...
		return(print_err(20500, 'Illegal characters (' + tmp2 + ') in the random part of the id: ' + id_chunk))
...
def verify_id_format(id, expected_prefix, version=1):
	'''verify_id_format(id, expected_prefix, version=1)
	...
	if expected_prefix is None:
		return(print_err(20600, 'There was no expected_prefix passed to verify_id_format.'))
	...
	if (id[0:3] != expected_prefix):
		return(print_err(20900, 'The prefix (' + id[0:3] \
	...
	if (expected_prefix in ('SID', 'SMD', 'MIX')):
		...
		if (len(id) == 35):
			...
			return(print_err(21000, 'Length of a shard ID must be 35. Observed length was: ' \
				+ str(len(id))))
			#raise Exception('Illegal ID length')
		...
		if (len(id) == 141):
			...
			if ( rc != 0):
				print_err(rc, 'The set of characters for this ID was illegal: ' + id)
				return(rc)
			...
			if (id[0:3] == 'PRV'):
				if (test_yyyymmdd != 40010101):
					...
					return(print_err(21200, "The expiration date for PRV IDs should be ' \
						+ '40010101, but this one is:" + str(test_yyyymmdd)))
			else:
				if (test_yyyymmdd > ID_CUTOFF_YYYYMMDD):
					return(print_err(21300, "The expire date for this account is too ' \
						+ 'far in the future zzz: " + str(test_yyyymmdd)))

				if (test_yyyymmdd < CURRENT_YYYYMMDD):
					return(print_err(1, "This ID expired on: " + str(test_yyyymmdd)))

				if (test_mm > 12):
					return(print_err(21500, "invalid month for ID: " + str(test_mm)))

				if (test_mm > 31):
					return(print_err(21600, "invalid day for ID: " + str(test_dd)))

		else:
			return(print_err(21700, 'Length of ID must be 141 (3-letter code + ' \
				+ '2-byte reserved + 8-byte date + ' \
				+ '65 bytes server fingerprint + 32 bytes random).	I found length=' \
...
class RNCrypt_bob(RNCryptor.RNCryptor):
	...
	Bob added 'decrypt_to_str' and set it 
	disable str conversion.
	"""
...
########################################################################
class shard_send_queue_args(object):
	"""
	...
	input_fpath=None, wrk_dir=None, add_proof_of_work=True):
		...
		# fpath is used for verification and loading before sending to the shard_send routine
...
class thread_shard_send(threading.Thread):
	...
	def run(self):
		"""
		This will push one shard to a shard server.
		...
		if self.wrk_dir is None:
			...
			self.qa.task_done() # tell the queue that the task is finished (doesn't help)
			nm_write_shard_status(self.status_fname, 'failed')
			return(print_err(23000, 'There was no wrk_dir sent to thread_shard_send.'))

		self.status_fname = os.path.join(self.wrk_dir, self.shard_id + '.status')
		nm_write_shard_status(self.status_fname, 'initializing')

		debug_msg(5, 'In thread_shard_send, status fname is ' \
			+ self.status_fname)
		...
		if self.cargo_bytes is None:
			...
			try:
				...
				return(print_err(23100, 'Failed to read the input_fpath (file ' \
					+ 'containing the shard to send) ' \
					+ 'for thread_shard_send: ' + str(self.input_fpath) + '. ' + self.e))

			self.fd_in.close()
			
		if not isinstance(self.cargo_bytes, bytes):
			self.err_msg = 'Cargo_bytes sent to thread_shard_send was not in ' \
				+ 'python bytes() format.'
			nm_write_shard_status(self.status_fname, 'failed',
				error_detail={'Error-detail': self.err_msg})

			self.qa.task_done() # tell the queue that the task is finished
			return(23400) #, self.err_msg))
		...
		if self.add_proof_of_work:
			self.nm_pow = create_shard_pow(self.cargo_sha1, len(self.cargo_bytes), pow_factor=.05,
			min_bits=2, bit_constant=0)

			debug_msg(5, 'In thread_shard_send, final pow is ' \
				+ self.nm_pow)

			self.url = self.web_host + '/shard_create?shard_id=' + self.shard_id \
				+ '&nm_pow=' + self.nm_pow
		else:
			self.url = self.web_host + '/shard_create?shard_id=' + self.shard_id

		##attached_files = {'shard_data': ('overrideinputfilenamegoeshere',
		self.attached_files = {'shard_data': ('shard_data',
					self.cargo_bytes,
					'application/x-download', {'Expires': '0'})}

		nm_write_shard_status(self.status_fname, 'sending')
			
		debug_msg( 5, 'making https request for url: ' + self.url \
		...
		try:
			...
			nm_write_shard_status(self.status_fname, 'failed', error_detail='The HTML ' \
				+ 'request failed. ' + self.err_msg)

			self.qa.task_done() # tell the queue that the task is finished
			return(23500) #, {'Error': self.err_msg}))
		...
		if self.r is not None:
			...
			if self.json_err_msg is not None:
				...
				nm_write_shard_status(self.status_fname, 'failed', error_detail='The server ' \
					+ 'returned an error while I was pushing a shard to a shard server: ' \
					+ str(self.json_err_msg))
				self.qa.task_done() # tell the queue that the task is finished
				return(23600) #, str(self.json_err_msg)))
			else:
				# The server returned JSON, but there was no error message (case sensitive).
				...
				try:
					...
					nm_write_shard_status(self.status_fname, 'failed', error_detail=self.err_msg)
					self.qa.task_done() # tell the queue that the task is finished
					return(23700) #, {'Error': err_msg}))
				...
				if self.svr_response is not None:
					...
					if self.err_response is not None:
						nm_write_shard_status(self.status_fname, 'failed', error_detail=self.err_response)
						self.qa.task_done() # tell the queue that the task is finished
						return(23800) #, dict(self.err_response)))
					else:
						try:
							status = self.r.json()['shard_create']['status']
						except:
							self.err_msg = 'Final status was not returned from the server.'
							nm_write_shard_status(self.status_fname, 'failed', error_detail=self.err_msg)
							self.qa.task_done() # tell the queue that the task is finished
							return(23900) #, {'Error', err_msg}))

						if status.upper() == 'OK':
							debug_msg(5,'=== Good shard status for ' + self.shard_id)
							# good  -- check for status=ok
							debug_msg( 4, 'In thread_shard_send, url was ' + self.url \
								+ ' officially good ')
							...
							self.qa.task_done() # tell the queue that the task is finished
							return(24000)# , {'Error', 'Final status returned from thread_shard_send was not good.'}))
				else:
					# The shard server returned something, but it could not be parsed into JSON.
					self.qa.task_done() # tell the queue that the task is finished
					return(24100) #, {'Error', 'The shard server returned JSON but it do not have the ' \
						#+ 'shard_create key in it: ' + str(r.json())}))
		else:
			self.qa.task_done() # tell the queue that the task is finished
			return(24200) #, {'Error', 'The request to the shard server did not fail, but there was a blank response.'}))
...
########################################################################
######################################################################
class shard_receive_queue_args(object):
	"""
	This class will hold data that is queued for the shard-read process.
	"""

	def __init__(self, web_host=None, shard_id=None, 
		out_dir=None, add_proof_of_work=True, output_fname=None):

		# fname is used for verification and loading before sending to the shard_send routine
		self.web_host=web_host
		self.shard_id=shard_id
...
class thread_shard_receive(threading.Thread):
	...
	def run(self):
		my_args = self.qa.get() # execute get() in the run routine, not in __init__
		...
		if self.out_dir is None:
			# I can not write the status to disk because there is no path
			# to the output directory.
			# To Do: this should probably raise an exception -- or catch the error
			# during the final status verification (detect missing file).
			self.qa.task_done() # tell the queue that the task is finished
			return(print_err(25000, 'There was no output directory  sent to thread_shard_send.'))


		self.status_fname = os.path.join(self.out_dir, self.shard_id + '.status')
		nm_write_shard_status(self.status_fname, 'initializing')
		...
		if not os.path.isdir(self.out_dir):
			try:
				...
				self.qa.task_done() # tell the queue that the task is finished
				return(print_err(25100, 'Could not create the directory to save shard data.'))
		...
		try:
			...
			self.qa.task_done() # tell the queue that the task is finished
			return(print_err(25200, 'The HTML request for the shard read failed. ' \
		...
		if self.r is not None:
			try:
				...
				self.e = str(sys.exc_info()[0:2])
				nm_write_shard_status(self.status_fname, 'failed', error_detail='Failed to save the shard to disk')
				...
				return(print_err(25300, 'Failed to send the shard to disk: ' + str(sys.exc_info()[0:2])))
			...
			nm_write_shard_status(self.status_fname, 'failed', error_detail='Response for HTML GET was None.')
			self.qa.task_done() # tell the queue that the task is finished
...
def nm_verify_server_files(nonce, nonce_signature, 
	...
	proof that the current online key (which should expire within a month or so),
	has been signed by the offline private key. If the server is overtaken
	...
	key that is not signed by an offline public key that has the SHA384 fingerprint
	that matches the fingerprint in the users box ID.

	This calls a compiled program called nm_sign that was created by Natural Message.
	The keys here are built using libgcrypt (which is the engine behind GnuPG), but
	the keys are stored as S-expressions that can be used by libgcrypt--the 
	keys are not in GnuPG/PGP format.
	"""
...
def create_shard_pow(payload_sha128, file_size, pow_factor, 
	...
	while not found:
		...
		#nonce_hex_str = base64.b16encode(os.urandom(int((target_bits + 2) / 8 + 1))).decode('utf-8').lower()
		nonce_hex_str = base64.b16encode(RNCryptor.Random.new().read(int((target_bits + 2) / 8 + 1))).decode('utf-8').lower()
		h = hashlib.sha1(b''.join([payload_sha128, bytes( yyyymmdd_str + nonce_hex_str, 'utf-8')])).digest()

		i = int().from_bytes(h, 'big')
		if mask & i == mask:
			debug_msg(7, 'Found a good proof of work key on iteration ' + str(j) \
					+ ' with mask\n' + bin(mask) + '\nand hash\n' + bin(i))
	...
	debug_msg(7, 'Found it on iter ' + str(j) + '. nonce =' + str(nonce_hex_str) \
			+ ' h =' + str(h) + ' elapsed time: ' + str(ellapsed_time))
		# Final report:
		# The user will submit the nonce_hex_str value as proof of work.
	debug_msg(7, 'Proof of work info: File size: ' + str(file_size) + ' bits: ' \
...
def xor_and_write(fname_prefix=None, msg_chunks=None, out_dir=None):
	...
	if type(msg_chunks)  != type([]):
		print('Error. The msg_chunks array was not of type array in xor_and_write.')
		return(27100)
	...
	fd_o = open( shard_fname, 'wb')
	#fd_o.write(base64.b64encode(msg_parity)) # old style before binary transfer
...
def nm_write_shard_status(fname, status, return_err_nbr=0,
	...
	if os.path.isfile(fname):
		...
		try:
			...
			output_d = json.loads(fd.read()) # load and make a dictionary object
			fd.close()
		except:
			e = str(sys.exc_info()[0:2])
			print_err(5850, 'Could not read the existing status file: ' + fname + '. ' + e)
			# keep going..
		if fd:
			fd.close()
		...
		try:
			...
			e = str(sys.exc_info()[0:2])
			return(print_err(5851, 'Could not update dictionary for: ' + fname + '. ' + e))
	...
	try:
		...
		return(print_err(28000, 'Could not write to the status file: ' + fname + '. ' + e))
	finally:
		os.fsync(fd2.fileno())
		fd2.close()
	...
	if not os.path.isfile(status_fname):
		...
		try:
			...
			try:
				...
				pass
			return((print_err(28200, 'Can not load the JSON from the status file: ' \
...
def show_shard_status(wrk_dir, shard_args, interval=3):
	"""
	For each queue_args entry in shard_args, get the shard_id and
	read .status files in the specified directory that correspond to
	the shard IDs in shard_args.shard_ID, and print a line that 
	...
	while not completed:
		...
		for sa in shard_args:
			...
			try:
				...
				try:
					...
					pass
				if loop_count > 1:
					return(print_err(28300, 'In show_shard_status, status file is not ready yet: ' \
						+ status_fname + '. ' + (e)))
			else:
				...
				if st_value is not None:
					if st_value in status_dict.keys():
						# If the current status value is already in my local status_dict,
						# update my local status_dict with the count.
						# 'st_value' is the value of the status, like 'failed' or 'sent', and
						# that becomes the key in status_dict and points to a count.
						status_dict.update({st_value: status_dict[st_value] + 1})
					else:
		...
		if len(status_dict) == 1:
			# xxx
			if 'sent' in status_dict.keys()  or  'received' in status_dict.keys() \
...
def nm_smd_create(web_host, dest_box_id,
	...
	if type(cargo_bytes) != type(b'00'):
		return((29100, {'Error': 'Cargo_bytes sent to nm_smd_create was not in ' \
			+ 'python bytes() format.'}))
...
########################################################################
########################################################################
def nm_gen_shard_id(v=None):
	"""
	This will generate a random, 128-bit shard ID that will
	be used as the destination address for shards that are
	pushed to shard servers.
	"""
	

	return('SID' + base64.b16encode(RNCryptor.Random.new().read(17)).decode('utf8').upper()[0:32])
...
def shard_xor(action, chunks_array=None, chunks_fname_array=None, 
		...
		for f in chunks_fname_array:
			if f != '':
				try:
					...
					return((print_err(29600, 'Could not read shard #' + str(idx) + ' in shard_xor. ' + e), None)) 
	...
	if chunks_array is not None:
		...
		debug_msg( 8, 'In shard_xor, CHUNKS contents are:\n' + repr(chunks_array))
		block_count = len(chunks_array)

		if action == 'r':
			if not isinstance(chunks_array[-1], bytes):
				return((print_err(29700, 'The last input chunk to shard_xor was not a bytes object.'), None))

			## cancelled feb23 2015 # For action 'r', the last shard will be the parity block,
			## cancelled feb23 2015 # and in the current version it has an extra layer of base64,
			## cancelled feb23 2015 # so undo the base64
			## cancelled feb23 2015 chunks_array[-1] = base64.b64decode(chunks_array[-1]) # no more b64 feb 23, 2014
		
		# Some error checking:
		for b in chunks_array:
			if not isinstance(b, bytes):
				return((print_err(29800, 'One of the chunks sent to shard_xor was not a bytes object.'), None))
		
			block_sizes.append(len(b))

			if b == b'':
				if action == 'c':
					return((print_err(29900, 'In shard_xor, a block has zero length for action=c.'), None))
			...
			if block_sizes[-1] != save_block_size:
				...
				# block length.
				# For action 'l', the next-to-the-last block length can be short.
				if action == 'c':
					if len(block_sizes) != len(chunks_array):
						# This is not the last chunk, so the sizes should be the same.
						return((print_err(30000, 'The chunks sent to shard_xor() had inconsistent lengths.'), None))
			
					if block_sizes[-1] > save_block_size:
						return((print_err(30100, 'A chunk sent to shard_xor() had was bigger than the first chunk'), None))
				else:
					# action == 'r'
					if b != b'' and len(block_sizes) != len(chunks_array) - 1:
						# This is not the next-to-last chunk, so the sizes should be the same.
						return((print_err(30200, 'The chunks sent to shard_xor() had inconsistent lengths.'), None))

					if b != b'' and block_sizes[-1] > save_block_size and (len(block_sizes) != len(chunks_array)):
						# The parity block at the end will be big due to tripple base64,
						# but otherwise it is an error.
						return((print_err(30300, 'A chunk sent to shard_xor() had was bigger than the first chunk'), None))
	...
	if recovered_shard_fname is not None:
		debug_msg( 4, 'Writing the recovered shard to: ' + recovered_shard_fname)
...
def nm_slice(bytes_in, shard_count=3):
	...
	debug_msg(6, 'Length of bytes version of text: final is ' + str(len(bytes_in)))
	...
	for j in range(shard_count):
		...
		debug_msg( 5, 'Start and end indexes (in Python format) for getting shards: ' \
				+ str(strt) + ', ' + str(end))
...
########################################################################
########################################################################
def nm_reassemble_shards(shard_fname_array, output_fname, 
	parity_version=1, delete_shard_for_testing=False):
	"""
	...
	debug_msg(4, '** nm_reassemble_shards fname array ' + str(shard_fname_array))
	...
	if delete_shard_for_testing:
		...
		debug_msg(1, '*** Deleting shard for testing: ' + str(delete_shard_for_testing) \
	...
	if failure_count == 1:
		...
		if shard_fname_array[-1] == '':
			return(31100) # can not reassemble due to one missing regular and missing parity
		else:
			parity_needed = True
	...
	if parity_needed:
		...
		# To do, add directory
		fname_recovered = tempfile.mktemp(prefix='natmsg-tmprcvd-') # I can not say /tmp because that is not valid on all OS

		err_nbr,  recovered_block = shard_xor(action='r', chunks_fname_array=shard_fname_array, 
	...
	for j in range(len(shard_fname_array) - 1):
		...
		try:
			...
			# xxxx i get this message when the file exists!! 
			# xxxx i get this message when the file exists!! 
			print('WARNING (this should be a fatal errror for this message): failed to open shard for reading ' \
			...
			try:
				...
				e = repr(sys.exc_info()[0:2])
				print('WARNING: failed to read shard from ' + shard_fname_array[j] \
					+ ' and stream to ' + output_fname + '. ' + e, file=sys.stderr )
				fd_in.close()
			else:
...
def nm_inbox_read(host, port_nbr, private_box_id=None, smd_id=None, save_dir=None):
	"""
	...
	if host[0:8] != 'https://':
		return(print_err(10802, 'The host should begin with https://, but it did not.'))
	...
	while not EOF and inbox_count < max_reads:
		...
		if r is not None:
			...
			try:
				# The SMD was wrapped in Base64 (CJ-style), so unwrap it and put it
		...
		if svr_json is None:
			return(print_err(31500, 'The server did not return JSON for a shard ' \
			...
			if 'inbox_read' not in svr_json.keys():
				# bad error
				print_err(31505, 'There was an error reading shard meta data. ' \
			...
			if err_msg is not None:
				...
				if svr_json is None:
					return(print_err(31800, 'The server did not respond.  URL was ' + url))
				else:
					# Verify the available keys--ignore junk SMD.
					# We absolutely need session_id and smd_id from the server.
					# Without this nothing will allow us to burn entries from the
					# inbox.
					...
					if not EOF:
						# Stop processing when I receive EOF... there will be nothing else
						# in the JSON if EOF is there.
						if 'session_id' not in svr_json['inbox_read'].keys():
							print('==WARNING: SESSION_ID NOT FOUND')
							#return(print_err(31900, 'The server did not include session_id ' \
						...
						if 'status' not in svr_json['inbox_read'].keys():
							...
							if svr_json['inbox_read']['status'] in ( 'EOF', 'EOF1', 'EOF2'):
								EOF = True
						#- - - - - - - - - - - - - - - - - - - - 
						# Save the smd here
...
def nm_secure_remove(fname, passes=3):
	...
	debug_msg(6, 'securely removing ' + fname) # ==once debug level
...
def nm_archiver2_attach_files(fname_list, fd_out, item_types, strip_path=True,
	v=None):
	"""
...
def nm_fetch_server_farm():
	...
	try:
		...
		sfarm_fname = MAIL_DIR + os.sep + 'settings' + os.sep + 'serverFarm.json'
	except:
		e = repr(sys.exc_info()[0:2])
		debug_msg(1, 'There was no sfarm_fname available. You might need ' \
	...
	try:
		...
		# Note this error could mean that the the firewall is blocking
		# ('Connection aborted.', OSError(113, 'No route to host'))
		return((print_err(35000, 'Failed to fetch server farm list. ' + e), None))
	...
	if r is not None:
		try:
			...
			e = repr(sys.exc_info()[0:2])
			return((print_err(35100, 'Failed to convert serverFarm list to a dictionary. ' \
				+ e), None))
		...
		if sfarm_fname is not None:
			...
			debug_msg(3, 'There was no sfarm_fname available, so there will ' \
				+ 'be no disk copy of the current server farm list.')

		return((0, server_dict))
	else:	
		e = repr(sys.exc_info()[0:2])
		return((print_err(35200, 'Failed get a valid response from the server farm.' \
			+ e)))
		...
		for d in conf:
			if d['server_type'].lower() == 'directory':
				if IPV4:
					...
					server_list.append(d['address'] + os.sep + str(d['port_nbr']))
	except:
		return(None)
...
def nm_display_menu_section(lst, start_idx, title='', 
	add_numbers=True):
	"""
...
def nm_menu_choice(lst, prompt=': ', title='', trim_width=True):
	...
	nm_display_menu_section(menu_txt_clean, idx, title=title, add_numbers=False)
	...
	while not good:
		...
		if choice.lower() == 'q':
			...
			nm_display_menu_section(menu_txt_clean, idx, title=title,
				add_numbers=False)
		elif choice.lower() == 'n':
			# Show prior page.
			idx +=  items_per_page
			...
			if int_choice is not None:
				if int_choice > len(lst) or int_choice < 1:
					print('Enter an integer FROM THE LIST (or Q to quit): ', end='')
				else:
					good = True
...
def nm_input_list_and_confirm(prompts, blanks_keep_default=False, trim_width=False ):
	"""
	...
	for t in prompts:
		...
		if len(t) != 3:
			return((print_err(36000, 'Each array entry sent to nm_input_list_and_confirm ' \
				+ 'should be a tuple with (a) the prompt text, (b) a boolean ' \
				+ 'to indicate if the entry must be integer (True = integer required) ' \
				+ 'and (c) a default value.'), None))
		tmp_answer = input_no_confirm(prompt=t[0], int_answer=t[1], default_value=t[2])
	...
	if quit_now:
		...
		good = False
		while good == False:
			choice, val = nm_menu_choice(prompts_with_answers,
				prompt='Enter a number to modify an entry, or press Q to finish: ',
				trim_width=trim_width)
		
			if choice < 0:
				good = True
			else:
				answers[choice] = input_no_confirm(prompts[choice][0] + ': ', prompts[choice][1])
				prompts_with_answers[choice] = prompts[choice][0] + ': ' + str(answers[choice])
...
def nm_set_pgm_opt(opt_key, pgm_list, prompt1, prompt2):
	...
	debug_msg(6, 'The existing, encrypted pgm path is: ' \
	...
	if MAIN_CONFIG['SETTINGS'][opt_key] == '':
		...
		for ppp in pgm_list:
			...
			paths.append( os.path.join(os.path.sep, 'usr', 'local', 'bin', ppp))
		...
		if MAIN_CONFIG['SETTINGS'][opt_key] == '':
			...
			if nm_confirm(prompt2):
				...
				try:
					...
					e = str(sys.exc_info()[0:2])
					print_err(37553, 'Could resolve the entry to a file name. ' + e)
		
				if os.path.isfile(path):
					# The thing that was entered is a real file name (presumably a program)
					options_changed = True
					the_pgm = None
					try:
...
def nm_set_rtf_reader_pgm():
	rc = nm_set_pgm_opt('rtf_reader_pgm',
				...
				prompt1='The RTF viewer program is not installed or is not in ' \
					+ 'the expected location.  It is not essential to have an RTF ' \
					+ 'viewer if you have the unrtf program (or textutil on Mac) ' \
				...
				prompt2='Do you want to manually enter a path to point ' \
					+ 'to the RTF viewer program (such as MS Word, OpenOffice...)? ' \
					+ '(y/n): ')
	return(rc)
...
### #	pgm_list = ['libreoffice', 'openoffice', 'abiword', 'catdoc', 'ashampoo']
### #def nm_set_rtf_reader_pgm(opt_key, pgm_list, prompt1, prompt2):
### def nm_set_pgm_opt(opt_key, pgm_list, prompt1, prompt2):
### 	"""
### 	This will be used to find a type of program (like a text editor or
### 	RTF editor) or prompt the user to enter the path to such a 
### 	program, then the response will be encrypted and saved in the
...
### 		MAIN_CONFIG['SETTINGS'][opt_key] = ''
### 
### 	if not os.path.isfile(MAIN_CONFIG['SETTINGS'][opt_key]):
### 		options_changed = True
### 		# remove a bad option
### 		MAIN_CONFIG['SETTINGS'][opt_key] = ''
### 
...
### 	if MAIN_CONFIG['SETTINGS'][opt_key] == '':
### 		for pgm in pgm_list:
### 			p = os.path.join(os.path.sep, 'usr', 'bin', pgm)
### 			if os.path.isfile(p):
### 				options_changed = True
### 				print('I found the program in ' + p)
### 				time.sleep(1)
### 				MAIN_CONFIG['SETTINGS'][opt_key] = p
### 				break
### 			else:
### 				p = os.path.join(os.path.sep, 'usr', 'local', 'bin', pgm)
### 				if os.path.isfile(p):
### 					options_changed = True
### 					print('I found the program in ' + p)
### 					time.sleep(1)
...
### 		if MAIN_CONFIG['SETTINGS'][opt_key] == '':
### 
### 			## print('The RTF viewer program is not installed or is not in ' \
### 			## + 'the expected location.  It is not essential to have an RTF ' \
### 			## + 'viewer if you have the unrtf program (or textutil on Mac) ' \
### 			## + 'installed. ' + os.linesep \
### 			## + 'On UNIX-like systems, you can search for the ' \
...
### 			print(prompt1)
### 
### 			## if nm_confirm('Do you want to manually enter a path to point ' \
### 			## 	+ 'to the RTF viewer program (such as MS Word, OpenOffice...)? ' \
### 			## 	+ '(y/n): '):
### 			if nm_confirm(prompt2):
### 				path = input('Enter the full path to the program: ' )
...
### 				if os.path.isfile(path):
### 					options_changed = True
### 					MAIN_CONFIG['SETTINGS'][opt_key] = path
### 				else:
### 					print('That is not a file: ' + path + '.  Try again.')
### 					
### 	if options_changed:
...
### 			fd.close()
### 		except:
### 			print_err(37556, 'Could not save the configuration file.  ' \
### 				+ 'This is a serious error.  ' \
### 				+ 'Check permissions to the home directory, and check free disk ' \
### 				+ 'space. The filename was ' + CONFIG_FNAME) 
### 			input('Press any key to continue...')
...
def nm_find_default_config_fname():
	...
	if need_new_config_file:
		debug_msg(4, 'need to config file in nm_find_default_config_fname')
		config_fname = None
		fd = None
	...
	if config_fname is None:
		debug_msg(2, '=== config_fname is None at the end of ' \
			+ 'nm_find_default_config_fname().' )
	else:
		debug_msg(4, '=== returning from nm_find_default_config_fname(): ' + config_fname)
...
def nm_create_config_file(homedir):
	...
	debug_msg(1,'Can not find a configuration file.  Creating a new one now.')
	...
	if CONFIG_FNAME is None:
		...
		return((print_err(36500, 'Could not resolve config file name.'), None))

	if CONFIG_FNAME == '':
		# should not execute unless programmer error.
		return((print_err(36600, 'Could not resolve config file name (blank).'),
			None))

	MAIL_DIR = os.path.expanduser('~' + os.sep + 'nm_mail')
	RECEIPT_FILE = MAIL_DIR + os.sep + 'settings' + os.sep + 'pbkdf2_receipt.save'
	...
	#
	if SESSION_PW == '':
		# The pw_hash2() function normally looks for the name of the receipt file
		# in the config file, but the config file does not exist.  Passing
		# the suggested filename will help pw_hash2() create the new receipt
		# in the right place.
	...
	if SESSION_PW is None:
		return((print_err(36800, 'Session password in nm_create_config_file ' \
			+ 'is None.'), None))

	if SESSION_PW == '':
		return((print_err(36900, 'Session password in nm_create_config_file ' \
			+ 'is blank.'), None))
	#
	...
	if err_nbr != 0:
		return((print_err(err_nbr, 'Failed to create a new box ID.  Try again in ' \
			+ '10 minutes.'), None))
	...
	config_txt = nm_decrypt_local_txt(MAIN_CONFIG['Identity1']['prvid'], SESSION_PW)
	if config_txt != prv_id:
		return((print_err(37000, 'Self-test on decryption of private ID failed.  This is ' \
			+ 'probably a programmer error.'), None))
	...
	if nm_confirm(prompt='Do you want to add another box ID under the ' \
		...
		if err_nbr != 0:
			print_err(err_nbr, 'Failed to create the second box ID under the ' \
				+ 'default identity. ' \
			...
			nickname = input_and_confirm('Enter a nickname for the second box ID (this is ' \
	...
	err_nbr, prv_id, pub_id = nm_account_create()
	if err_nbr != 0:
		return((print_err(err_nbr, 'Failed to create a new box ID.  Try ' \
			+ 'again in 10 minutes.'), None))
	...
	if nm_confirm(prompt='Do you want to add another box ID under the ' \
		...
		if err_nbr != 0:
			...
			nickname = input_and_confirm('Enter a nickname for the second box ' \
				+ 'ID under Top Secret ' \
	...
	if err_nbr != 0:
		return((print_err(37100, 'Could not save the configuration file.  ' \
			+ 'This is a serious error.  ' \
			+ 'Check permissions to the home directory, and check free disk space.'), None))

	print('')
	print('==============================================================================')
	print('WARNING: It is SUPER IMPORTANT that you make a backup copy of your configuration')
	print('file: ')
	print('    ' +  CONFIG_FNAME)
	print('That file contains encrypted information that is needed to access your inbox.')
	...
	print('the old one.')
	print('WARNING: If you forget your password, you will not be able to access your inbox.')
	print('==============================================================================')
	input('Press any key to continue....')


	nm_set_unrtf_pgm()
	##nm_set_pgm_opt('unrtf_pgm', ['unrtf'], prompt1='The unrtf program is ' \
	##	+ 'not installed or is not in the expected location.',
	##	prompt2='Do you want to manually enter ' \
	##	+ 'a path to point to the unrtf program? (y/n): ')
		...
		if root_owner_found:
			...
			for root, dirs, files in os.walk(MAIL_DIR):
				for f in files:
					if f == '':
						# Fix directory owner
						shutil.chown(root, user=owner_numeric_id, group=owner_gid )
					else:
						# Fix regular file owner
						fpath = os.path.join(root, f)
						debug_msg(2, '==== fixing owner for ' \
								+ fpath)
						shutil.chown(fpath, user=owner_numeric_id, group=owner_gid  )
...
def nm_write_config(v=None):
	...
	if CONFIG_FNAME is not None:
		try:
			...
			print('Warning.  Could not save an archive copy of the configuration file.')
		...
		try:
			...
			return(print_err(37500, 'Could not save the configuration file.  ' \
				+ 'This is a serious error.  ' \
				+ 'Check permissions to the home directory, and check free disk ' \
				+ 'space. The filename was ' + CONFIG_FNAME) )
	else:
			return(print_err(37600, 'Could not determine the filename for savingthe MAIN_CONFIG file.'))
...
def nm_create_contact_entry(public_box_id, nickname=None, name=None, 
	...
	if name is not None:
		ctc += ',"name":"' + base64.b64encode(bytes(name, 'utf-8')).decode('utf-8') + '"'
	...
	if notes is not None:
		ctc += ',"notes":"' + base64.b64encode(bytes(notes, 'utf-8')).decode('utf-8') + '"'
...
############################################################
########################################################################
def nm_build_contact_dict(current_identity, include_anonymous=False,
	local_only=False):
	"""
	...
	if not local_only:
		for a in MAIN_CONFIG[current_identity].keys():
			# Loop through the contacts under this identity, format some information,
			# then add a key/value pair for contact_description and {id_nbr: THENBR, box_id=THE_BOX_ID}
			if a[0:21] == 'contact_public_box_id':
				id_nbr = int(a[21:])
				# Decrypt the nickname
				try:
					nickname = ' ' \
						+ nm_decrypt_local_txt(MAIN_CONFIG[current_identity]['contact_nickname' \
				...
				try:
					contact_note = ' ' \
						+ nm_decrypt_local_txt(MAIN_CONFIG[current_identity]['contact_note' \
						+ str(id_nbr)], SESSION_PW)
				...
				contact_description = nickname + ', ' + box_id[0:16] + '...' + box_id[-6:]
				# contact description and the full box ID:
				contact_dict.update({contact_description: {'id_nbr': id_nbr, 'box_id':box_id, 
					'nickname':nickname, 'contact_note':contact_note}})
	...
	for a in MAIN_CONFIG[current_identity].keys():
		if a[0:5] == 'pubid':
			...
			try:
				...
				nickname = '*' + nm_decrypt_local_txt(MAIN_CONFIG[current_identity]['nickname' \
					+ str(id_nbr)], SESSION_PW)
	...
	if include_anonymous:
		contact_dict.update({contact_description: {'id_nbr': 0, 'box_id':'Anonymous'}})
...
def nm_start(batch=False):
	...
	if not need_new_config_file:
		if tmp_config_fname != '':
			...
			# there was an error
			return(38100, 'There was an error trying to determine the config ' \
				+ 'file name.')
	else:
		pass # delete the old logic after testing
		...
		#### if not os.path.isdir(os.path.expanduser(os.path.join('~', 'Downloads'))):
		#### 	os.makedirs(os.path.expanduser(os.path.join('~', 'Downloads'))) # do not alter the privileges
	...
	if need_new_config_file:
		...
		if err_nbr != 0:
			return(print_err(38200, 'Failed to save the initial configuration file.'))
		else:
	...
	if CONFIG_FNAME is None:
		return(print_err(38300, 'After the configuration file was supposed to ' \
			+ 'be verified, there ' \
	...
	if 'screen_width' not in MAIN_CONFIG['SETTINGS'].keys():
		print('WARNING. Your options file does not contain an entry for ' \
	...
	if 'screen_height' not in MAIN_CONFIG['SETTINGS'].keys():
		...
		MAIN_CONFIG['SETTINGS']['screen_height'] = '20' # all values are strings
		time.sleep(.3)
	##-#-#-#-#-#- 
	...
	if 'mail_dir' not in MAIN_CONFIG['SETTINGS'].keys():
		print('Error.  Your settings file is corrupt. There is no value for ' \
			+ 'mail_dir.')
		print('Either restore ' + CONFIG_FNAME + ' or rename it to force this ' \
			+ 'program to create a new file.  If you let this program generate ' \
			+ 'a new settings file, ' \
			+ 'will create a new box ID for you (you must recover your old ' \
			+ 'settings file if you want to receive messages to your old box_id).')
		print('If you create a new settings file, use the same password so that ' \
			+ 'you can copy your old contacts into the new file when you restore ' \
			+ 'your real config file.')
		sys.exit(333)
	else:
		MAIL_DIR = MAIN_CONFIG['SETTINGS']['mail_dir']
	#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
	...
	## some verification
	if 'screen_width' not in MAIN_CONFIG['SETTINGS'].keys():
		print('WARNING. Your options file does not contain an entry for screen_width. ' \
			+ 'Defaulting to 80')
		MAIN_CONFIG['SETTINGS']['screen_width'] = '80' # all values are strings
		time.sleep(.3)

	if 'screen_height' not in MAIN_CONFIG['SETTINGS'].keys():
		print('WARNING. Your options file does not contain an entry for screen_height. ' \
			+ 'Defaulting to 20')
		MAIN_CONFIG['SETTINGS']['screen_height'] = '20' # all values are strings
		time.sleep(.3)

	if 'current_identity' not in MAIN_CONFIG['SETTINGS'].keys():
		print('WARNING. Your options file does not contain an entry for your ' \
			+ 'current identity.  Defaulting to Identity1.')
		MAIN_CONFIG['SETTINGS']['current_identity'] = 'Identity1'
		time.sleep(.7)

	if 'verbosity' not in MAIN_CONFIG['SETTINGS'].keys():
		print('WARNING. Your options file does not contain an entry for verbosity.' \
			+ 'Defaulting to 4 (which is higher than usual because you obviously ' \
			+ 'have some problems).')
	...
	if 'download_directory' not in MAIN_CONFIG['SETTINGS'].keys():
		...
		print('WARNING. Your options file does not contain an entry for the download ' \
	...
	if not batch:
		if SESSION_PW == '':
			SESSION_PW = pw_hash2(receipt_fname=MAIN_CONFIG['SETTINGS']['PBKDF2_receipt_file'])
			if SESSION_PW is None:
	...
	if receipt_fname is None:
		try:
			...
			if not os.path.isfile(receipt_fname):
				...
				print('WARNING: Could not get the name of the password receipt file from the ' \
					+ ' configuration table.  The file is usually in your home directory and ' \
					+ ' called natmsgsc.ini or .natmsgsc. I wil put the file in')
				print('You can try to continue or try to restore the configuration file.')
				junk = input('Press any key to continue....')
	else:
		if not os.path.isfile(receipt_fname):
			need_new_receipt = True
		...
		if pw_hashed is not None:
			if not need_new_receipt:
				...
				with open(receipt_fname, 'r') as fd:
					...
					saved_receipt = nm_decrypt_local_txt(encrypted_receipt, pw_hashed)
				...
				print('')
				print('If you are starting the client application for the first time,')
				...
				if nm_confirm:
					if not os.path.isdir(os.path.dirname(receipt_fname)):
						...
						try:
							os.makedirs(os.path.dirname(receipt_fname), mode=0o700)
						except:
							e = str(sys.exc_info()[0:2])
							print_err(39000, 'Failed to create a receipt for your login password: ' \
								+ receipt_fname + '.  Did you change your option file or alter your ' \
								+ 'home directory? Check your mail_dir option and pbkdf2_receipt_file ' \
								+ 'option in ' + CONFIG_FNAME +'\nThe original error was: ' + e)
							print_err(39001, 'Another possibility is that the path shown ' \
								+ 'above came from a ' \
								+ 'config file that you copied from another computer, ' \
								+ 'and the directory ' \
								+ 'names in your .natmsg config file are wrong.')
							return(None)
					...
					# FIX THIS. I DON'T HAVE A PASSWORD ON THE FIRST RUN
					# FIX THIS. I DON'T HAVE A PASSWORD ON THE FIRST RUN
					receipt = nm_encrypt_local_txt(static_receipt_txt, pw_hashed)
					...
					junk = input('...')
					pw_hashed_new = pw_hash2(iterations=iterations, receipt_fname=receipt_fname)
					if pw_hashed_new != pw_hashed:
						pw_hashed = None # force another loop

		else:
			print('There might be a problem with the pw_hash routine.  You can ' \
				+ 'try again or kill the program.')
			input('Press any key to continue...')
...
def roll_gdg(file_path):
	...
	if os.path.isfile(file_path):
		...
		for x in gdg_entries:
			debug_msg(6, 'In roll_gdg, found existing GDG member: ' + str(x))
			if x > highest_gdg:
				highest_gdg = x
...
def nm_select_editor():
	...
	if platform.system().lower() in ['darwin', 'windows']:
		...
		if len(found_editors) == 0:
			return(print_err(93459, 'Failed to obtain a choice for the editor.'))
		else:
			# There was at least one 'found editor' in the list.
			idx, choice = nm_menu_choice(found_editors, title='Select the path and ' \
				+ 'program name for a text editor that you want to use.')
		
			if idx < 0:
				return(print_err(9345, 'Failed to obtain a choice for the editor.'))
			else:
...
def nm_clean_utf8_text(data_raw):
	...
	if not isinstance(data_raw, bytes):
		try:
			...
			data_raw = bytes('[The original message contained invalid data]', 'utf-8')
	...
	if data_raw[0:5] == bytes('{\\rtf', 'utf-8'):
		...
		debug_msg(4, 'Calling the unrtf program to strip RTF in the main message.')
			...
			if pgm_encrypted == '':
				...
				if unrtf_pgm is not None:
					if os.path.isfile(unrtf_pgm):
						try:
							pid = subprocess.Popen([unrtf_pgm, '--text', '--quiet', '--nopict'],
							stdin=subprocess.PIPE,
							stdout=subprocess.PIPE, stderr=subprocess.PIPE)
					
							# 'unrtf_out' is a python bytes() object because input is binary.
							unrtf_out, err_msg = pid.communicate(fd.read())
							# Standardize newlines first!! 
							# (there could be side-effects if the text contains invalid 
							# binary data).
						except:
							e = str(sys.exc_info()[0:2])
							show_warning = True
	...
	if show_warning:
		print('WARNING: Could not run the unrtf program to remove RTF codes.  ' \
			+ 'RTF codes might appear in the output.' )
		print('If you install the unrtf program, you can point to it using the ' \
	...
	if unrtf_out is not None:
		...
		if data_array[0][0:len(UNRTF_HDR)] == bytes(UNRTF_HDR, 'utf-8'):
			...
			try:
				...
				debug_msg(3, 'The RTF data produced non-UTF-8 characters, so some ' \
...
############################################################
# Choose a File
def nm_file_chooser(top, prompt='Select a file or directory to read: ', \
	mode='file', select_msg_files=False):
	"""
	...
	if mode not in ['file', 'directory']:
		natmsgblib.print_err(3929, 'Invalid mode for nm_file_chooser. Must be ' \
			+ 'file or directory.')
		return(None)
		...
		while not finished:
			for root, dirs, files in os.walk(top):
				if root == top:
					...
					# The 'answer' will be just the filename without the full path.
					...
					if answer in dirs:
						if mode=='file':
							...
							# Directory mode
							a = input_no_confirm('Enter S to select this directory or G to go to it: ')
							if a is not None:	
								if a.lower() == 's':
...
def nm_search_files(top_directory, search_string):
	"""
	This is a case insensitive search of the message and metadata JSON to find files
	to list in the 'inbox search' screen.
	"""
